    ...
    <div x-data="{open:false}" class="bg-white divide-y divide-slate-200 px-4 py-2">
        <div @click="open=!open" class="cursor-pointer flex justify-between px-4 py-4">
            <div>
                ...
                <p class="font-extralight text-sm italic">{{__("Vous ajoutez actuellement des projets pour l'épreuve : ")}} <span class="">{{$this->jiri->name}}</span></p>
            </div>
                    ...
                    <div>
                        <div class="mb-4">
                            ...
                            <datalist id="projectsList">
                                @foreach($this->filteredAvailableProjects($this->jiri->id) as $project)
